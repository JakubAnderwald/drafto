...
| In Test     | factory                        | PR's Vercel preview is deployed and reachable. PR remains open; awaits human approval. **Allowlisted-reporter exception**: factory auto-advances to Approved (skips this gate) when the issue carries `reporter-allowlisted: true` in its support-agent footer and the migration gate passes. |
...
| Existing job                                                | Change                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ----------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `support-agent.sh` (every 5 min)                            | Real-time email→issue role unchanged. Two new behaviours: (1) when filing from an **allowlisted** reporter (per `$SUPPORT_ALLOWLIST` in `logs/support-state.json` — Jakub and his wife), it auto-applies `status:ready` and adds the issue to the Projects v2 board. Non-allowlisted reports land in Backlog as today. (2) New classifier output `accept-signal`: when an allowlisted reporter replies on a thread linked to a factory issue with a positive intent ("thanks", "ship it", `[ACCEPT]`), support-agent runs `state-cli.mjs factory:done <issue>` to advance the card and close the issue. Non-allowlisted replies still go through as comments only. |

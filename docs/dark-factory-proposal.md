...
| In Progress | human or factory (allowlisted) | Plan approved; factory has taken a worktree slot and Claude is implementing per the approved plan. Human drags card here from Plan Review, OR an allowlisted reporter sends a "go ahead" reply on the linked support thread (support-agent's accept-signal classifier advances on their behalf), OR the **In Test iteration loop** returns it here when a reporter comments a change request on the preview (the next `--implement` revises the existing PR in place).                                                                                                                                                                                                  |
...
| In Test     | factory                        | PR's Vercel preview is deployed and reachable. PR remains open; awaits human approval **or change requests**.<br/><br/>**In Test iteration loop**: a reporter comment requesting changes (anything that isn't pure approval/noise, newer than the consumed-feedback high-water mark) returns the card to In Progress for an in-place revision on the same PR branch + preview URL; it loops back to In Test. Approval stays the explicit Approved drag.<br/><br/>**Allowlisted-reporter exception**: factory auto-advances to Approved (skips this gate) when the issue carries `reporter-allowlisted: true` in its support-agent footer and the migration gate passes. |
...
1. **Approval gate** — branches by reporter:
   ...
   - **Change requests instead of approval**: while the card is in In Test, a
     reporter comment requesting changes (not pure approval/noise) runs the

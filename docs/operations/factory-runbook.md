...
| Worktree install hangs       | Each `pnpm install` is wrapped by `run-with-timeout.mjs` and capped at `FACTORY_INSTALL_TIMEOUT_SEC` (default 600 s); on cap the slot + worktree are released and the card's attempt budget is bumped. A cold install should take seconds (clonefile seed + offline reconcile), not minutes — see "Worktree installs & disk".                      |

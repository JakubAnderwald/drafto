...
- Optional: `E2E_ADMIN_EMAIL`, `E2E_ADMIN_PASSWORD` and `SUPABASE_SERVICE_ROLE_KEY` together enable the `chromium-admin` project, which runs `apps/web/e2e/admin.spec.ts`. Unless all three are set, that project isn't registered and the admin specs don't run. CI passes the three only on `main` (a push, or a manual run from `main`) and never to pull-request runs, because those execute unmerged code and the service-role key bypasses RLS. So in CI the admin specs run only on `main`, and only once the matching repository secrets exist. The service-role key must be the **dev** project's: `/admin` builds a service-role client on every render and errors without it. The account must be a **separate** user in the dev project with both `profiles.is_approved = true` and `profiles.is_admin = true` — don't make the `E2E_TEST_EMAIL` user an admin, or every other spec sees the admin UI.

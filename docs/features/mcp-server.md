...
Drafto exposes a remote Model Context Protocol (MCP) server at `/api/mcp`, letting Claude Desktop, Claude Cowork, claude.ai, and any other MCP client list, read, search, create, update, move, and trash notes and to create, list, rename, and delete notebooks on behalf of an authenticated Drafto user.

## Current state

The server is live in production at `https://drafto.eu/api/mcp` and listed on the public MCP Registry under the namespace `eu.drafto/mcp`. Authentication is via user-generated API keys managed at `/settings`. Eleven tools are registered in the server — see the enumeration below.
...
| `rename_notebook` | Rename a notebook owned by the caller (mirrors `PATCH /api/notebooks/[id]`).         | `{ notebook_id: string (uuid), name: string 1..100 }`                             | JSON object `{ id, name }` of the renamed notebook. Trims the name and rejects an empty name; an owner-scoped existence check returns `Error: Notebook not found` for a missing/not-owned notebook.                        |
| `delete_notebook` | Delete an empty notebook owned by the caller (mirrors `DELETE /api/notebooks/[id]`). | `{ notebook_id: string (uuid) }`                                                  | Plain-text confirmation: `Notebook "<name>" deleted.` Refuses with a clear error if the notebook still holds non-trashed notes; returns `Error: Notebook not found` for a missing/not-owned notebook.                      |
...
- `rename_notebook` and `delete_notebook` mirror `PATCH` / `DELETE /api/notebooks/[id]`. Both run an explicit owner-scoped existence check first, because an `UPDATE` / `DELETE` on a missing or not-owned row affects 0 rows without raising a Supabase error — without the pre-check the tool would falsely report success. `delete_notebook` also mirrors the web route's guard refusing to delete a notebook that still holds non-trashed notes.
- Attachment upload and permanent _note_ delete are **not** exposed as tools by design (see ADR 0017).
...
- `apps/web/__tests__/unit/mcp-tools.test.ts` — handler-level tests for each of the eleven tools.

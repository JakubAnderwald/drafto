...
- **macOS.** "Delete account" (`delete-account-row`) sits next to "Sign out" in the sidebar's user section (`notebooks-sidebar.tsx`). The first click opens `DeleteAccountPanel` inline under the two actions, and its confirm button is the second click. The panel uses the same test IDs as mobile.
  ...
  - Offline (same `isConnected` / `isInternetReachable === false` rule as mobile), the row is disabled and shows the offline note, and the panel's confirm is disabled too. On `ok`, `app-navigator.tsx` renders the login screen once the session is null.

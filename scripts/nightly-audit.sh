...
# Stable signature of today's problem set, independent of the date. Identical
# problem sets across days collide on the same hash, so we can detect "still
# occurring" vs "new problem appeared / one cleared". NUL-terminate so each
# problem (which contains embedded newlines) stays a single sort unit, and
# sort -z to keep the comparison order-insensitive.
PROBLEM_SIG=$(printf '%s\0' "${PROBLEMS[@]}" | LC_ALL=C sort -z | shasum | cut -c1-12)
SIG_MARKER="<!-- audit-signature: $PROBLEM_SIG -->"
...
ISSUE_BODY+=$'\n'
ISSUE_BODY+="$SIG_MARKER"

# Idempotency: if an open audit issue already carries this signature, the
# problem set is unchanged — comment "still occurring" instead of filing a
# duplicate. Otherwise file fresh. Previously this script created a new issue
...
if [[ -n "$MATCHING_NUM" ]]; then
  ...
  gh issue comment "$MATCHING_NUM" --repo "$REPO" \
    --body "Still occurring on $TODAY. (Same problem set; see \`logs/audit-$TODAY.log\`.)" \
    >/dev/null || log "WARNING: failed to comment on #$MATCHING_NUM"
else
  log "Creating audit issue (signature $PROBLEM_SIG)."
  gh issue create --repo "$REPO" \
